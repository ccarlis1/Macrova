...
- Documented in `../../planner/parity-debugging.md` (append a "Failure modes" section).
- Tests:
...
- `src/planning/planner.py` — `plan_meals` return type (`MealPlanResult`); identify the `report` field structure and where `termination_code` is set (phase10_reporting.py)
...
- `../../planner/parity-debugging.md` — append a "Failure modes" section
...
5. **Read `../../planner/parity-debugging.md`.** Find the right place to append the "Failure modes" section.
...
- `../../planner/parity-debugging.md` has a new "Failure modes" section
