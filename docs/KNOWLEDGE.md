...
- Calorie Deficit Mode: There should be a feature in place to set a hard cap at the maximum amount of calories that should be allowed for the day. This would be a useful tool for people in a strict calorie deficit, this feature would make that much easier. There should be a hard penalty for a meal combination that has its total calories over this value. This should be an optional value to add in the user XML configuration file.
## Macronutrients
