...
**Ingredient Classification for Carb Scaling:** When `enable_primary_carb_downscaling` is `true`, each ingredient in a recipe may be classified as a **scalable carb source** if its normalized name matches an entry in one of two maintained reference lists:

  

- **Rice variants:** rice, white rice, brown rice, jasmine rice, basmati rice, cream of rice, wild rice, and similar cultivar-specific names.

- **Potato variants:** potato, potatoes, sweet potato, sweet potatoes, russet potato, gold potato, yukon gold potato, red potato, and similar cultivar-specific names.

  

The exhaustive lists are maintained as reference data (see Section 2.3). An ingredient not on either list is not eligible for scaling regardless of its carbohydrate content.

  

Within a recipe, the **primary carb source** is defined as the single scalable carb source ingredient that contributes the most carbohydrate grams to the recipe's total nutrition. If no ingredient qualifies, the recipe has no primary carb source and is ineligible for scaling. If a recipe contains multiple scalable carb source ingredients, only the single highest-carbohydrate contributor is designated as primary; the others are not scaled. Ingredients marked `is_to_taste = true` are excluded from consideration.

  

**Primary Carb Contribution data requirement:** Recipes eligible for Primary Carb Downscaling shall include a precomputed field `primary_carb_contribution` of type `NutritionProfile` (or equivalent nutritional vector), representing the full nutritional contribution (calories, protein, fat, carbs, and all micronutrients) of the primary carb source ingredient at its original quantity. This field is precomputed prior to planner execution and stored alongside the recipe's total `nutrition` profile. Recipes that do not possess a primary carb source (Section 2.2) need not include this field. The algorithm reads `primary_carb_contribution` as input; it does not derive it at planning time.
...
| `micronutrients_consumed` | Dict[str, float] | Sum of each micronutrient from all recipes assigned to day d so far. This dictionary includes **all** micronutrients that appear in recipe nutrition profiles — not only those in `U.micronutrient_targets` ("tracked" micronutrients), but also any micronutrient that has a resolved UL. This broader scope is required because UL enforcement (HC-4, FC-3) is independent of `micronutrient_targets` (see Section 4, HC-4). |
...
### 6.7 Primary Carb Downscaling (Optional Feature)
...
**Activation:** This feature is active only when `U.enable_primary_carb_downscaling = true`. When inactive (`false` or unset), the entirety of this section has no effect on the algorithm.
...
The Primary Carb Downscaling operator generates reduced-carb **scaled variants** of candidate recipes that would otherwise be rejected due to calorie excess (HC-5 or FC-1). Its purpose is to reduce unnecessary backtracking by fitting a nutritionally suitable recipe into the day's remaining calorie budget through a bounded reduction of its primary carb source quantity.

  

The operator is conservative: it modifies only a single ingredient in a controlled, bounded manner, and all variants must independently satisfy every hard constraint before being admitted as candidates.
...
The operator shall be applied to a recipe r at decision point `(d, s)` only when **all** of the following conditions are met:

  

1. `U.enable_primary_carb_downscaling = true`.

2. The slot's `activity_context` contains `sedentary`. If the slot's `activity_context` contains `pre_workout` or `post_workout`, the operator shall not apply, regardless of other flags present.

3. Recipe r was rejected during candidate generation (Section 6.3) **specifically due to calorie excess**: either an HC-5 violation (assigning r would exceed `max_daily_calories`) or an FC-1 calorie overflow failure.

4. Recipe r possesses a **primary carb source** as defined in Section 2.2: a scalable carb source ingredient (rice variant or potato variant) that is the single highest carbohydrate contributor in the recipe, is not `is_to_taste`, and has a non-zero quantity.

5. The slot `(d, s)` is **not** a pinned assignment. Pinned assignments (HC-6) are fixed and shall not be modified in any form.

  

If any precondition is not met, the operator does not apply to that recipe at that decision point. The recipe remains rejected and no variants are generated.

  

#### 6.7.3 Scaling Mechanics

  

For a recipe r with primary carb source ingredient at original quantity `q_original`, the operator generates up to **K** scaled variants, where:

  

- **K** = `U.max_scaling_steps` (configurable, default: 4)

- **σ** = `U.scaling_step_fraction` (configurable, default: 0.10)

  

Variant i (for i = 1, 2, ..., K) sets the primary carb source quantity to:

  

`q_i = q_original × (1 − i × σ)`

  

**Lower bound constraint:** Every variant shall satisfy `q_i > 0`. The configuration shall satisfy `K × σ < 1.0`. If this invariant is violated, the effective K is reduced to the largest integer k such that `k × σ < 1.0`. Any step i where `q_i ≤ 0` shall not be generated.

  

With default parameters (K = 4, σ = 0.10), the variants are:

  

| Variant | Quantity | Fraction of Original |

|---|---|---|

| 1 | `q_original × 0.90` | 90% |

| 2 | `q_original × 0.80` | 80% |

| 3 | `q_original × 0.70` | 70% |

| 4 | `q_original × 0.60` | 60% |

  

**Only the primary carb source ingredient is modified.** All other ingredients in the recipe retain their original quantities. The recipe's cooking time, instructions, and all non-nutritional attributes are unchanged.

  

#### 6.7.4 Nutrition Recalculation

  

After scaling, each variant's total nutrition profile shall be recomputed to reflect the modified primary carb source quantity:

  

- Let `contribution_original` = the full nutritional contribution (calories, protein, fat, carbs, and all micronutrients) of the primary carb source ingredient at quantity `q_original`.

- Let `contribution_scaled` = the nutritional contribution at quantity `q_i`, computed proportionally: `contribution_scaled = contribution_original × (q_i / q_original)`.

- Variant total nutrition = `r.nutrition − contribution_original + contribution_scaled`.

  

This recalculation applies to **all** nutritional fields: calories, protein, fat, carbs, and every micronutrient contributed by the primary carb source ingredient. The nutritional contributions of all other ingredients are unchanged.

  

The per-ingredient nutritional contribution of the primary carb source is available via the precomputed `primary_carb_contribution` field on the recipe (see Section 2.2). Specifically: `contribution_original = r.primary_carb_contribution`.

  

#### 6.7.5 Variant Evaluation

  

Each generated variant is evaluated against all hard constraints and feasibility checks, identically to any standard candidate recipe:
...
- **HC-5 (Calorie Deficit Mode):** Rechecked with the variant's recalculated calories. This is the primary constraint the operator is designed to satisfy.

- **FC-1, FC-2, FC-3:** Rechecked with the variant's recalculated nutrition profile.

  

Only variants that pass **all** hard constraints and feasibility checks are added to the candidate set `C(d, s)`. Variants that still violate constraints are discarded.

  

#### 6.7.6 Variant Identity and Determinism

  

A scaled variant of recipe r is identified by the tuple `(r.id, i)` where `i` is the scaling step (1 through K). For the purposes of:

  

- **HC-2 (Recipe Uniqueness Per Day):** A scaled variant counts as the same recipe as the original. Assigning any variant `(r.id, i)` to a slot on day d prevents the original recipe r and all other variants of r from being assigned to another slot on day d.

- **HC-8 (Consecutive-Day Repetition):** A scaled variant inherits the original recipe's ID and is subject to the same consecutive-day constraints.

- **Scoring (Section 8):** Each variant is scored independently using its recalculated nutrition profile through the same cost function as all other candidates.

- **Heuristic ordering (Section 7):** Variants compete alongside all other candidates (including the unscaled original, if it is also in the candidate set) in the scoring and tie-breaking order. The algorithm may select any variant or the original based solely on score.

  

The operator is **deterministic**: given the same recipe, the same configuration parameters, and the same state, the same set of variants is generated in the same order.

  

#### 6.7.7 Interaction with Backtracking
...
If no variants pass evaluation (all remain in calorie excess or violate other constraints), the candidate set is unaffected. If `C(d, s)` is still empty, BT-1 triggers and standard backtracking rules (Section 9) apply.

  

The operator does not introduce additional backtracking triggers. It does not modify the backtracking procedure. It does not apply retroactively to meals already assigned to earlier slots — it operates only at the current decision point during forward candidate generation.

  

#### 6.7.8 Constraints on the Operator

  

1. **Reduction only:** The operator shall only reduce quantity, never increase. All variants satisfy `q_i < q_original`.

2. **Strictly positive quantity:** All variants satisfy `q_i > 0`. The primary carb source is never reduced to zero and never removed from the recipe.

3. **Single ingredient only:** Only the primary carb source ingredient is scaled per recipe. No other ingredients are modified.

4. **Eligible sources only:** Only ingredients classified as rice variants or potato variants (per the reference lists in Section 2.3) are eligible. No other carbohydrate sources (bread, pasta, oats, tortillas, etc.) shall be scaled.

5. **Sedentary slots only:** The operator shall not apply to any slot whose `activity_context` includes `pre_workout` or `post_workout`.
...
- Two pinned assignments on the same day use the same recipe ID (violates HC-2).
...
| **Scaled variant** | A copy of a recipe with the primary carb source quantity reduced by a defined scaling step. Identified by `(recipe_id, scaling_step)`. Treated as the same recipe for HC-2 and HC-8 purposes. |
...
All open items have been resolved. See Appendix C for the full resolution history.
